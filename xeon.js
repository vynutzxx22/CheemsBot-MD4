...
async function startXeonBotInc() {
    ...
    XeonBotInc.sendMessage(callerId, { text: `Sistem Otomatis Blokir!\nJangan Menelpon Bot!\nSilahkan Chat Owner Untuk Dibuka Blokirnya!\nKontak Yang Dikirim Ini Bukan Bot, Tapi Orang Yang Mempunyai Bot Ini\nChat Untuk Buat Stiker Atau Yang Lain Akan Langsung Diblokir`}, { quoted : xeonfek })
    await sleep(8000)
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/c00a072bb31aa594bfece.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/c00a072bb31aa594bfece.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                let xeonName = num
                const xtime = moment.tz('Asia/Jakarta').format('HH:mm:ss')
              const xdate = moment.tz('Asia/Jakarta').format('DD/MM/YYYY')
              const xmembers = metadata.participants.length
                XeonBotInc.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `┌─❖
│「 𝗛𝗶 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
│ 🥳  𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 
├ ★ ${metadata.subject}
...
└───────────────┈ ⳹`} )
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await XeonBotInc.getName(i + '@s.whatsapp.net')}\nFN:${global.ownername}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Click here to chat\nitem2.URL;type=INTERNET:${global.socialm}\nitem2.X-ABLabel:My Instagram\nitem3.URL:${global.ytname}\nitem3.X-ABLabel:My Web\nitem4.ADR:;;${global.location};;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
